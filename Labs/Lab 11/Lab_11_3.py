...
class BallDraw(object):
    def __init__ (self, parent):
        ...
        self.L = []
        for i in range(10):  
            self.L.append(Ball(random.randint(0, 400), random.randint(0, 400), random.randint(-8, 8), random.randint(-8, 8), random.randint(5, 10), random.choice(colorList)))     
    ...
    def animate(self):
        ...
        while self.ball.some_inside(self.maxx, self.maxy):
            ...
            for i in range(10):
                self.L[i].draw_ball()
                self.L[i].ball.move()  
                self.L[i].ball.check_and_reverse(self.maxx, self.maxy)
